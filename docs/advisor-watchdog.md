...
- `mutation` — skip a mid-turn boundary only when every tool call in the not-yet-reviewed messages is review-exempt. The exempt set is derived from [`READ_ONLY_TOOL_NAMES`](../packages/coding-agent/src/task/read-only-policy.ts) minus `retain`, `memory_edit`, `checkpoint`, and `rewind` (read-tier, but they mutate durable state), leaving `read`, `wait`, `grep`, `glob`, `find`, `ast_grep`, `web_search`, `ask`, `todo`, `recall`, `reflect`, and `yield`. It is an allowlist, never a mutating-tool list: `task`, `lsp` (whose `rename`/`code_actions` edit files), `bash`, `eval`, `edit`, `write`, `ast_edit`, `github`, memory writes, and every MCP/plugin tool trigger a review, so an unrecognized tool costs one extra review instead of creating a blind spot. That includes `write` to `agent://` (a peer message) and to `proc://` (cancelling a job, stopping or feeding a service): a peer message needs no user confirmation, but steering a peer or killing a job is exactly the mid-flight decision an advisor should see. Reading `proc://` or `agent://` is a plain `read`, so it is exempt. An unrecognized `advisor.reviewOn` value (a hand-edited typo) behaves like `step`. The scan covers everything since the review cursor, not just the newest step.
