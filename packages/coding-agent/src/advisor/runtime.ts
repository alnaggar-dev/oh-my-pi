...
export class AdvisorRuntime {
	 ...
	 * @param opts.shouldReview - Mid-turn review gate over the transcript and the
	 *   review cursor. Only consulted while `willContinue` is `true`: the terminal
	 *   boundary is always reviewed. Omitted → every step is reviewed.
	 */
	onTurnEnd(
		messages?: AgentMessage[],
		opts?: { willContinue?: boolean; shouldReview?: (all: readonly AgentMessage[], from: number) => boolean },
	): void {
		...
		if (wip && opts?.shouldReview && !opts.shouldReview(all, this.#lastCount)) return;
