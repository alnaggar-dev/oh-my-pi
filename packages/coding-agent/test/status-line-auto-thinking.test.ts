 ...
 * turns it resolved (and, after a `!`, turns that fell back to a guess), plus a
 * pending marker while a classification is in flight. Both renderers show it,
 * and the custom bar's render cache must invalidate when the tallies move: the
 * session hands out one mutated activity object, so a stable object reference
 * alone would freeze the counters at their first painted value.
 */
